...
```
SELECT seller_name FROM Sellers s WHERE NOT EXISTS ( SELECT category_id FROM Categories c WHERE NOT EXISTS ( SELECT * FROM Products p WHERE p.seller_id = s.seller_id AND p.category_id = c.category_id ) );
```